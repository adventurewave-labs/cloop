...
//!
//! The serializer deliberately supports only a flat subset of TOML — top-level
//! `key = value` pairs, basic strings, integers, booleans, and triple-quoted
//! multi-line strings. That keeps the parser ~80 lines and dependency-free.
//! Assumption: a multi-line value never contains a line that is exactly `"""`.
...
pub enum StopKind {
    ...
    Iterations,
    /// Stop when Claude prints a marker string.
    Marker,
    /// Stop when a shell command exits 0.
    Command,
}
...
impl StopKind {
    pub fn as_str(self) -> &'static str {
        match self {
            StopKind::Iterations => "iterations",
            StopKind::Marker => "marker",
            StopKind::Command => "command",
        }
    }
    pub fn from_str(s: &str) -> StopKind {
        match s {
            "iterations" => StopKind::Iterations,
            "marker" => StopKind::Marker,
            _ => StopKind::Command,
        }
    }
}
...
impl Default for LoopConfig {
    fn default() -> Self {
        LoopConfig {
            ...
            workdir: ".".to_string(),
            model: String::new(),
            stop_kind: StopKind::Command,
            stop_command: String::new(),
            stop_marker: "LOOP_DONE".to_string(),
            max_iterations: 25,
            carry_context: true,
            ...
            iter_prompt: String::new(),
        }
    }
}

/// The built-in per-iteration prompt template for a given stop mode.
...
/// Build the argument vector for one `claude` invocation.
pub fn claude_args(cfg: &LoopConfig, iteration: u64, prompt: &str) -> Vec<String> {
    let mut args: Vec<String> = Vec::new();
    args.push("--print".to_string());
    if iteration > 1 && cfg.carry_context {
    ...
    for a in split_args(&cfg.extra_args) {
        args.push(a);
    }
...
own line and nothing after it.",
    ...
    let template = if cfg.iter_prompt.is_empty() {
    ...
    template
        ...
        .replace("{max}", &cfg.max_iterations.to_string())
        .replace("{stop_command}", &cfg.stop_command)
        .replace("{stop_marker}", &cfg.stop_marker)
        .replace("{check_output}", check_output)
        .replace("{task}", &cfg.task)
}

/// Split a string into arguments, honoring single and double quotes. Quotes
/// group tokens but are not themselves included in the output.
...
pub fn to_toml(cfg: &LoopConfig) -> String {
    ...
    out.push_str(
        "# Placeholders: {iteration} {max} {task} {stop_command} {stop_marker} {check_output}\n",
    );
...
/// Parse our TOML subset. Unknown keys are ignored; missing keys use defaults.
pub fn from_toml(text: &str) -> Result<LoopConfig, String> {
    ...
    while let Some(raw) = lines.next() {
        ...
        if val_part == "\"\"\"" {
            // Multi-line: collect until a line that is exactly """.
    ...
    if let Some(v) = map.get("task") {
        cfg.task = v.clone();
    }
    if let Some(v) = map.get("workdir") {
        if !v.is_empty() {
            cfg.workdir = v.clone();
        }
    }
    ...
    if let Some(v) = map.get("stop_kind") {
        cfg.stop_kind = StopKind::from_str(v);
    }
    if let Some(v) = map.get("stop_command") {
        cfg.stop_command = v.clone();
    }
    if let Some(v) = map.get("stop_marker") {
        if !v.is_empty() {
    ...
    if let Some(v) = map.get("carry_context") {
        cfg.carry_context = parse_bool(v, cfg.carry_context);
    }
    if let Some(v) = map.get("permission_mode") {
        cfg.permission_mode = v.clone();
    }
    if let Some(v) = map.get("skip_permissions") {
    ...
    if let Some(v) = map.get("delay_secs") {
        if let Ok(n) = v.trim().parse::<u64>() {
            cfg.delay_secs = n;
        }
    }
    if let Some(v) = map.get("extra_args") {
        cfg.extra_args = v.clone();
    }
    if let Some(v) = map.get("iter_prompt") {
...
/// A human-readable, colored summary block (shown before saving / running).
pub fn summary(cfg: &LoopConfig) -> String {
    ...
    let task_disp = if cfg.task.lines().count() > 1 {
        format!("{} {}", task_first, ui::dim("(…)"))
    } else {
        task_first
    };
    ...
    rows.push((
        "carry context".into(),
        if cfg.carry_context { "yes" } else { "no" }.into(),
    ));
    let perms = if cfg.skip_permissions {
        ui::red("bypass (--dangerously-skip-permissions)")
    } else if cfg.permission_mode.is_empty() {
        "default".to_string()
    } else {
    ...
    if cfg.max_turns > 0 {
        rows.push(("max turns".into(), cfg.max_turns.to_string()));
    }
    if !cfg.allowed_tools.is_empty() {
        rows.push(("allowed tools".into(), cfg.allowed_tools.clone()));
    }
    if cfg.delay_secs > 0 {
        rows.push(("delay".into(), format!("{}s", cfg.delay_secs)));
    }
    if !cfg.extra_args.is_empty() {
        rows.push(("extra args".into(), cfg.extra_args.clone()));
    }
