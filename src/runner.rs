...
pub fn run(cfg: &LoopConfig, dry_run: bool) -> Result<i32, Box<dyn Error>> {
    ...
    for iteration in 1..=cfg.max_iterations {
        ...
        ui::step(&format!("iteration {} / {}  {}", iteration, cfg.max_iterations, ui::dim(&format!("[{}]", elapsed(start)))));
        println!("  {}", ui::dim(&display_cmd(&args)));
        println!();
        let mut child = match Command::new("claude").args(&args).current_dir(dir)
            .stdin(Stdio::null()).stdout(Stdio::piped()).stderr(Stdio::inherit()).spawn() {
            Ok(c) => c,
            Err(e) if e.kind() == std::io::ErrorKind::NotFound => {
                return Err("`claude` was not found on your PATH. Install Claude Code first: https://docs.claude.com/en/docs/claude-code".into());
            }
        ...
        if !status.success() {
            ui::warn(&format!("claude exited with {} (continuing)", status.code().map(|c| c.to_string()).unwrap_or_else(|| "a signal".into())));
        }
        println!();
        match cfg.stop_kind {
            StopKind::Marker => {
                if captured.contains(&cfg.stop_marker) {
                    ui::success(&format!("marker '{}' seen on iteration {} ({})", cfg.stop_marker, iteration, elapsed(start)));
                    return Ok(0);
                }
            }
            StopKind::Command => {
                ui::info(&format!("running check: {}", cfg.stop_command));
                let out = Command::new("sh").arg("-c").arg(&cfg.stop_command).current_dir(dir).output()?;
                let mut combined = String::from_utf8_lossy(&out.stdout).into_owned();
                combined.push_str(&String::from_utf8_lossy(&out.stderr));
                check_output = combined;
                if out.status.success() {
                    ui::success(&format!("check passed on iteration {} ({})", iteration, elapsed(start)));
                    return Ok(0);
                } else {
    ...
    match cfg.stop_kind {
        StopKind::Iterations => {
            ui::success(&format!("completed {} iterations ({})", cfg.max_iterations, elapsed(start)));
            Ok(0)
        }
        _ => {
            ui::error(&format!("reached the {}-iteration cap without satisfying the stop condition", cfg.max_iterations));
            Ok(1)
        }
    }
}
