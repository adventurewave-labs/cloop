...
fn sanitize_name(s: &str) -> String {
    ...
    if trimmed.is_empty() { "loop".to_string() } else { trimmed }
}
...
pub fn run_wizard(default_name: &str) -> io::Result<LoopConfig> {
    ...
    println!();
    let models = ["default (Claude Code's configured model)", "sonnet", "opus", "haiku", "custom…"];
    let m = ui::ask_select("Model", &models, 0)?;
    cfg.model = match m { 0 => String::new(), 1 => "sonnet".into(), 2 => "opus".into(), 3 => "haiku".into(), _ => ui::ask_text("Model name", None)? };
    println!();
    let stops = ["command — loop until a shell command exits 0 (e.g. tests pass)", "marker — loop until Claude prints a marker string", "iterations — run a fixed number of times"];
    let s = ui::ask_select("Stop condition", &stops, 0)?;
    cfg.stop_kind = match s { 1 => StopKind::Marker, 2 => StopKind::Iterations, _ => StopKind::Command };
    match cfg.stop_kind {
        StopKind::Command => { cfg.stop_command = ui::ask_text("Check command", Some("cargo test"))?; }
        StopKind::Marker => { cfg.stop_marker = ui::ask_text("Marker string", Some("LOOP_DONE"))?; }
        StopKind::Iterations => {}
    }
    println!();
    let cap_label = if cfg.stop_kind == StopKind::Iterations { "How many iterations?" } else { "Max iterations (safety cap)" };
    ...
    println!();
    let perms = ["default — Claude asks before risky actions", "acceptEdits — auto-accept file edits", "plan — planning only, no changes", "dontAsk — unattended-safe, fewer prompts", "bypassPermissions — skip ALL checks (containers/VMs only)"];
    ...
    match p {
        ...
        _ => {
            ui::warn(&ui::red("bypassPermissions removes all safety checks. Use only in a sandbox."));
    ...
    cfg.iter_prompt = if customize { ui::ask_multiline("Enter your per-iteration prompt")? } else { default_prompt };
    Ok(cfg)
}
